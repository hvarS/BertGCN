...
from model.models import BertGCNTorch
from utils import *
import dgl
...
# for elem in [adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask]:
...
if gcn_model == 'gcn':
    model = BertGCNTorch(nb_class=nb_class, pretrained_model=bert_init, m=m, gcn_layers=gcn_layers,
...
#build PyG graph
adj_norm = normalize_adj(adj + sp.eye(adj.shape[0]))
edge_indices,edge_weights = gutils.from_scipy_sparse_matrix(adj_norm.astype('float32'))
node_features = th.zeros((nb_node, model.feat_dim))
g = GraphData(x=node_features,edge_index=edge_indices,edge_attr=edge_weights,y=th.LongTensor(y))
g.input_ids = input_ids
g.attention_masks = attention_mask
g.train_mask,g.val_mask ,g.test_mask  = th.FloatTensor(train_mask), th.FloatTensor(val_mask), th.FloatTensor(test_mask)
g.label_train = th.LongTensor(y_train)


# print(graph.num_node_features,graph.num_edge_features,graph.num_edges,graph.num_nodes)
...
def update_feature():
    ...
    dataloader = Data.DataLoader(
        Data.TensorDataset(g.input_ids[doc_mask], g.attention_masks[doc_mask]),
        batch_size=1024
    )
    with th.no_grad():
...
def train_step(engine, batch):
    ...
    optimizer.zero_grad()
    train_mask = g.train_mask[idx].type(th.BoolTensor)
    y_pred = model(g, idx)[train_mask]
    y_true = g.label_train[idx][train_mask]
